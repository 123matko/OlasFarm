...
function onMessageArrived(message) {
  ...
  if (message.destinationName == "OlasFarm/level") {
    ...
    if (actual_level == "start") {
      window.open("./" + text.loadLevel + "/level1_2.html", "_self");
    } else {
      window.open("../" + text.loadLevel + "/level1_2.html", "_self");
    }
  }
      ...
      if (y > 420 && y < 460) {
        switch (actual_level) {
          case "animals":
            ...
            break;
          case "wool":
            if (expectedword == "Seno") {
              gussed_wool();
            }
            break;
          case "kitchen":
            if (expectedword == "Tanier") {
              guessed_kitchen("Tanier");
            } else if (expectedword == "Mixer") {
              guessed_kitchen("Mixer");
            }
            break;
        }
      }
...
function show_start() {
  window.open("../start_2.html", "_self");
}
...
function gussed_wool() {
  if (mqttClient.isConnected()) {
    say("Správne");
    var content = JSON.stringify({ najdene: "Ihlice" });
    var message = new Paho.MQTT.Message(content);
